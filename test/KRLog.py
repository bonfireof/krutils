...
class _config:
    ...
    def __init__(self):
        ...
        # 시스템 로그를 포함하여 출력
        self.DEBUG_LEVEL_ALL    = _util().get_log_level_index("ALL");
        # # DB접근 로그를 포함하여 출력
        self.DEBUG_LEVEL_DB     = _util().get_log_level_index("DB");
        # # 프로그램 로그를 포함하여 출력
        self.DEBUG_LEVEL_APP    = _util().get_log_level_index("APP");
        # # 중요 정보 발생시 출력
        self.DEBUG_LEVEL_INFO   = _util().get_log_level_index("INFO");
        # # 오류 발생시 출력
        self.DEBUG_LEVEL_ERR    = _util().get_log_level_index("ERR");
        ...
        # self.CURR_DEBUG_LEVEL       = _util().get_log_level_index("ERR");
        self.CURR_DEBUG_LEVEL       = _util().get_log_level_index("ALL");
...
class _logging:
    ...
    def _gen_log_header(self, debug_level) -> str:
        ...
        if (debug_level == self.config.DEBUG_LEVEL_ALL):
            ...
            header = header + "[ INFO ]"
        elif (debug_level == self.config.DEBUG_LEVEL_ERR):
            header = header + "[ERROR ]"
...
class logger(_logging):
    ...
    def __init__(self, caller_path: str):
        ...
        # 존재시 설정을 덮어 씌운다
        self.config.CONFIG_FILE_PATH = _util()._find_config_file_path(caller_path)
    ...
    def error(self, template="", *args):
        self._print_log(self.config.DEBUG_LEVEL_ERR, template, *args)
